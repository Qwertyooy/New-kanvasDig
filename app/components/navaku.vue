...
// Fungsi Baru: Menangani klik navigasi, scroll mulus, dan tutup menu mobile
...
<template>  
    ...
    <header :class="[
        'animate-tech-in fixed top-0 left-0 right-0 z-50 flex justify-between items-center w-full transition-all duration-300 border-b',
        isDarkSection 
      ...
      <div class="flex items-center mx-4 w-full justify-between p-2">
        ...
        <nav class="py-2 items-center hidden md:flex font-mono">
          <a
              ...
              :class="[
                'relative inline-block px-5 text-xs uppercase tracking-widest bg-gradient-to-r bg-[length:0%_100%] bg-left-bottom bg-no-repeat transition-all duration-500 ease-out hover:bg-[length:100%_100%]', 
                isDarkSection 
                  ? 'text-white/70 from-white/10 to-white/10 hover:text-white' 
                  : 'text-ink/70 from-ink/10 to-ink/10 hover:text-ink'
              ]"
          >
              {{ link.title }}
          </a>
        ...
        <!-- Tombol "lets collab" -->
        <button :class="[
          'px-5 py-1.5 rounded-xl font-mono hidden md:flex items-center gap-2 text-xs uppercase tracking-wider transition-all duration-300 border backdrop-blur-sm active:scale-95 shadow-sm', 
          isDarkSection 
            ? 'bg-white/10 text-white border-white/20 hover:bg-white/20 hover:border-white/40' 
            : 'bg-black/5 text-ink border-black/10 hover:bg-black/10 hover:border-black/20'
        ]"> 
    ...
    <div 
        :class="[
            'fixed inset-0 z-40 flex flex-col items-center justify-center transition-all duration-500 ease-in-out md:hidden backdrop-blur-xl', 
    ...
    >
        <nav class="flex flex-col items-center gap-6 w-full max-w-sm px-6 font-mono">
            <a
                ...
                :class="[
                  ...
                  isDarkSection ? 'text-white/60 hover:text-white' : 'text-ink/60 hover:text-ink'
                ]"
            >
                {{ link.title }}
            </a>
        ...
        <button 
          @click="isOpen = false"
          :class="[
            'mt-8 px-6 py-2 rounded-xl font-mono text-xs uppercase tracking-wider transition-all duration-300 border backdrop-blur-sm shadow-md', 
            isDarkSection 
              ? 'bg-white/10 text-white border-white/20' 
              : 'bg-black/5 text-ink border-black/10'
          ]"
        > 
